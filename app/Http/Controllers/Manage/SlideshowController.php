...
class SlideshowController extends Controller
{
    ...
    public function index_data()
    {
        ...
        return DataTables::eloquent($rows)
            ->editColumn('slider_image', function ($row) {
                $image = '<img src="';
                $image .= $row->slider_image != null ? asset("assets/img/sliders/" . $row->slider_image) : "http://via.placeholder.com/930x390?text=SliderPhoto(930x390)";
    ...
    public function save()
    {
        ...
        if(request()->hasFile('image')){
            ...
            $path = public_path('assets/img/sliders/' . $filename);
            $square = Image::canvas(1000, 419, array(255, 255, 255));
