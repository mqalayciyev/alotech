...
@section('content')
    ...
    <section class="content-header">
        <h1 class="pull-left">Ölçü vahidləri</h1>
        <div class="pull-right">
            <button class="btn btn-success" id="add_data">
                <i class="fa fa-plus"></i> Yeni ölçü vahidi
            </button>
        </div>
                                ...
                                <div class="modal-content">
                                    <form method="post" id="form">
                                        ...
                                        <div class="modal-header">
                                            ...
                                            <h4 class="modal-title">Yeni ölçü vahidi</h4>
                                        </div>
                                        <div class="modal-body">
                                            <span id="form_output"></span>
                                            <div class="form-group">
                                                <label for="number">Nömrə</label> <br>
                                                <input type="text" name="number" class="form-control"
                                                       id="number" placeholder="Nömrə"
                                                       value="{{ old('number') }}">
                                            </div>
                                            <div class="form-group">
                                                <label for="name">Ölçü vahidi</label> <br>
                                                <input type="text" name="name" class="form-control"
                                                       id="name" placeholder="Ölçü vahidi"
                                                       value="{{ old('name') }}">
    ...
    <script>
        $(function () {
            ...
            $('#index_table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: '{{ route('manage.measurement.index_data') }}',
                columns: [
            ...
            $('#form').on('submit', function (event) {
                ...
                $.ajax({
                    method: 'POST',
                    url: '{{ route('manage.measurement.post_data') }}',
                    data: form_data,
                    dataType: 'json',
                    success: function (data) {
                        if (data.error.length > 0) {
                            ...
                            $('#form')[0].reset();
                            $('#action').val('Saxla');
                            $('.modal-title').text('Yeni ölçü vahidi');
                            $('#button_action').val('insert');
            ...
            $(document).on('click', '.edit', function () {
                var id = $(this).attr('id');
                $.ajax({
                    url: '{{ route('manage.measurement.fetch_data') }}',
                    method: 'GET',
                    data: {id: id},
                    dataType: 'json',
                    success: function (data) {
            ...
            $(document).on('click', '.delete', function () {
                ...
                if (confirm('{{ __('admin.Are you sure you want to delete this data?') }}')) {
                    $.ajax({
                        url: '{{ route('manage.measurement.delete_data') }}',
                        method: 'GET',
                        data: {id: id},
                        success: function (data) {
                            alert(data);
            ...
            $(document).on('click', '#bulk_delete', function () {
                ...
                if (confirm('{{ __('admin.Are you sure you want to delete this data?') }}')) {
                    ...
                    if (id.length > 0) {
                        $.ajax({
                            url: '{{ route('manage.measurement.mass_remove') }}',
                            method: 'GET',
                            data: {id: id},
                            success: function (data) {
                                alert(data);
