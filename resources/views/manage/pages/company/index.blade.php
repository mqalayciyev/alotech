...
@section('content')
    ...
    <section class="content-header">
        <h1 class="pull-left">Kompaniyalar</h1>
        <div class="pull-right">
            <a href="{{ route('manage.company.new') }}" class="btn btn-success">Yeni Kompaniya</a>
        </div>
    ...
    <script>
        $(function() {
            ...
            $('#index_table').DataTable({
                ...
                pageLength: 50,
                paging: true,
                ajax: '{{ route('manage.company.index_data') }}',
                columns: [{
                        data: 'order',
                        searchable: false
                    },
            ...
            $('tbody').sortable({
                ...
                update: function() {
                    ...
                    $.ajax({
                        url: "{{ route('manage.company.reorder') }}",
                        type: "post",
                        data: {
                            serialize: serialize
                        },
            ...
            $(document).on('click', '.delete', function() {
                ...
                if (confirm('{{ __('admin.Are you sure you want to delete this data?') }}')) {
                    $.ajax({
                        url: '{{ route('manage.company.delete_data') }}',
                        method: 'GET',
                        data: {
                            id: id
                        },
            ...
            $(document).on('click', '#bulk_delete', function() {
                ...
                if (confirm('{{ __('admin.Are you sure you want to delete this data?') }}')) {
                    ...
                    if (id.length > 0) {
                        $.ajax({
                            url: '{{ route('manage.company.mass_remove') }}',
                            method: 'GET',
                            data: {
                                id: id
                            },
